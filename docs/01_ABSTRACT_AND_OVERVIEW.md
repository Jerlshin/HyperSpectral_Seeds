...
> is derived from `src/spectralquadnet/`, `configs/`, `train.py`, `tests/` and the committed run
> artifacts in `outputs/output_v12_spa40/`. Where a quantity is *not* reproducible from this
> repository, it is marked as such rather than reported.
...
> **Provenance caveat.** The figures below are regenerated from the committed artifacts in
> `outputs/output_v12_spa40/`, which were produced under the **pre-Tier-3** architecture — a
...
| Gate | What it pins |
|---|---|
| `tests/regression/test_golden_forward_pass.py` | eval-mode logits, per-tensor SHA-256 of the initialised state dict (352 tensors), exact Stage-1 epoch-1 loss and post-step weight hashes, against the schema-v3 architecture |
...
`(seed, epoch)`. What *is* pinned and tested regardless of configuration: weight initialisation,

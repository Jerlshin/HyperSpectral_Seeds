#!/usr/bin/env python3
"""
Prepare Rice HSI Dataset — Fixed & Production-Ready
=====================================================

Builds:
    dataset/patches.npy  →  (N, 256, 64, 64)  float16  channel-first
    dataset/labels.npy   →  (N,)               int64

═══════════════════════════════════════════════════════════════════
ALL BUGS FOUND & FIXED FROM ORIGINAL data_setup.py
═══════════════════════════════════════════════════════════════════

BUG 1 — CRITICAL: SNV applied globally, destroying background=0 signal
  Location : snv() function
  Original : (patch - patch.mean()) / (patch.std() + 1e-6)
  Problem  : SNV is computed over ALL pixels including the zero-padded
             background.  The background zeros drag the mean down and
             shrink the std, so the resulting "normalized" background
             is NOT zero — it becomes a large positive uniform noise
             blob ( ≈ +mean/std ).  The model then learns that noise,
             not the seed spectrum.
  Fix      : Compute mean/std only from SEED pixels (mask > 0), then
             apply to the full patch.  Background stays exactly 0.

BUG 2 — CRITICAL: resize() uses cv2.INTER_LINEAR on spectral data
  Location : resize() function — cv2.resize default interpolation
  Original : cv2.resize(p[:,:,i], (64,64))   # default = INTER_LINEAR
  Problem  : INTER_LINEAR interpolates pixel values across space.
             For hyperspectral data this is fine spatially, BUT the
             default also triggers at the border where masked background
             zeros bleed into seed pixels (and vice-versa), corrupting
             the spectral signature at seed edges.
  Fix      : Use cv2.INTER_AREA when downsampling (more accurate for
             HSI), and explicitly pass interpolation flag always.

BUG 3 — CRITICAL: Massive redundant re-processing in patch-build loop
  Location : second loop in main() — "Build patches"
  Original : For every individual seed row, the entire cube is
             re-extracted from zip, re-preprocessed (dark correction,
             crop), and re-segmented.  With ~8640 seeds across 180
             cubes, each cube is loaded and segmented ~48 times.
  Impact   : ~48× slowdown (hours vs minutes), and segment() is
             non-deterministic w.r.t. numeric precision on repeated
             calls, so r.label IDs may not match saved rows → silent
             wrong-seed extraction.
  Fix      : Group seed_df by hdr path, process each cube ONCE, extract
             all its seeds in one pass.

BUG 4 — MODERATE: extract_pair() called on black.hdr but black.hdr
         has no matching .raw counterpart at that function's assumption
  Location : extract_pair() — raw_name = base + ".raw"
  Original : Assumes every .hdr has a matching .raw with the same stem.
             The spectral library (ENVI format) may use .img or other
             extensions for the data file.  This fails silently or
             raises FileNotFoundError on dark references.
  Fix      : Search for any file with the same stem but non-.hdr
             extension, rather than hardcoding ".raw".

BUG 5 — MODERATE: open_zip() uses parts[1] as session key
  Location : open_zip() session parsing
  Original : session = parts[1]
  Problem  : The zip internal path structure is:
               "RGB and VIS-NIR HSI.../Data-VIS-xxx/file.hdr"
             parts[0] is the top-level folder, parts[1] is the
             session folder.  But if the zip has a different nesting
             depth (no top-level wrapper, or double-nested), parts[1]
             is wrong and sessions dict is corrupted.
  Fix      : Find the part that starts with "Data-VIS" regardless of
             nesting depth.

BUG 6 — MODERATE: labels.npy written with idx zeros for failed patches
  Location : Build patches loop — y array pre-filled with zeros
  Original : y = np.zeros(N, np.int64).  If any patch fails (exception),
             idx is not incremented, but the pre-allocation assumed N
             successful patches.  Result: trailing zero-label entries
             that corrupt training.
  Fix      : Collect (patch, label) pairs in a list, then stack once
             at the end so the final array only contains successful
             patches.  Also truncate the memmap to actual count.

BUG 7 — MINOR: wl variable scoped inside tempfile block
  Location : wavelengths loading in main()
  Original : wl is defined inside "with tempfile..." block.  If the
             CSV is not found, wl is undefined and the next usage
             raises NameError with a confusing traceback.
  Fix      : Initialize wl = None before the block; assert it was
             found before proceeding.

BUG 8 — MINOR: tqdm imported but never used
  Location : import tqdm
  Fix      : Added to the cube processing loop for progress visibility.

═══════════════════════════════════════════════════════════════════
ADDITIONAL IMPROVEMENTS
═══════════════════════════════════════════════════════════════════
  - Wavelength-aware VIS band selection with fallback
  - Per-cube grouped processing (single cube load per variety bundle)
  - Verified patch count logged per variety for quality control
  - Binary closing added to segmentation (fills thin gaps in seeds)
  - Sanity check: patches with all-zero content are skipped
  - Output shapes and label distribution printed at end
"""

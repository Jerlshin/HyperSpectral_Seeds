"""
hsi_band_selection.py
=====================
Research-correct hyperspectral band selection for 90-class rice seed
classification (256-band VIS-NIR, 64×64 spatial patches).

Pipeline
--------
1. Mean spectra extraction   — chunked, foreground-masked
2. Decorrelation pre-filter  — removes near-duplicate bands (|r| > 0.995)
3. Fisher Discriminant Ratio — per-band separability (diagnostic)
4. mRMR selection            — Min-Redundancy Max-Relevance (MID criterion)
5. SPA selection             — Successive Projections Algorithm
6. StratifiedKFold validation— LDA + LinearSVC accuracy at each band count
7. Elbow detection           — smallest k reaching ≥ 98 % of peak accuracy
8. Save reduced patches      — chunked write of optimal subset
...
CONFIG = {
    ...
    "corr_threshold":  0.995,        # Drop band if |r| > this with any kept band

    # Band selection
    "n_select_max":    100,          # Rank up to this many bands in mRMR / SPA
    "n_candidates":    [5, 10, 15, 20, 25, 30, 40, 50, 70, 100],
    ...
    # Validation
    "cv_folds":        5,
    "svc_C":           0.1,          # Conservative C; avoids overfit on mean spectra
    "elbow_pct":       0.98,         # Fraction of peak accuracy used for elbow
...
# =====================================================================
def extract_mean_spectra() -> tuple[np.ndarray, np.ndarray]:
    """
    ...
    patches = np.load(CONFIG["patches_path"], mmap_mode='r')
    labels  = np.load(CONFIG["labels_path"])
    N, C, H, W = patches.shape
    X = np.zeros((N, C), dtype=np.float32)

    for s in tqdm(range(0, N, CONFIG["chunk_size"]), desc="       chunks"):
...
# =====================================================================
def decorrelation_prefilter(X: np.ndarray) -> np.ndarray:
    """
...
# =====================================================================
def fisher_discriminant_ratio(X: np.ndarray, y: np.ndarray,
                               wl_df: pd.DataFrame) -> np.ndarray:
    """
...
# =====================================================================
def run_mrmr(X: np.ndarray, y: np.ndarray,
    ...
    """
    print(f"\n[4/6]  mRMR greedy selection  (up to {CONFIG['n_select_max']} bands)...")
    ...
    rel = mutual_info_classif(
        ...
        n_neighbors=CONFIG["mi_neighbors"],
    ...
    n_select  = min(CONFIG["n_select_max"], P)
...
# =====================================================================
def run_spa(X: np.ndarray, candidates: np.ndarray,
            init_global: int) -> np.ndarray:
    """
    ...
    """
    print(f"\n[5/6]  SPA selection  (up to {CONFIG['n_select_max']} bands)...")
...
# =====================================================================
def validate(X: np.ndarray, y: np.ndarray,
             ordered: np.ndarray, label: str) -> dict:
    """
    For each k in CONFIG['n_candidates'], evaluate two classifiers using
    ...
    counts = [k for k in CONFIG["n_candidates"] if k <= len(ordered)]
    result = {}

    print(f"\n       Validating [{label}]  ({CONFIG['cv_folds']}-fold StratifiedKFold)...")
    ...
    for k in tqdm(counts, desc=f"       {label}"):
        ...
        # LDA
        try:
            lda_acc = float(cross_val_score(
                make_pipeline(
                    StandardScaler(),
                    LinearDiscriminantAnalysis(solver='svd', tol=1e-4),
                ),
                Xs, y, cv=skf, scoring='accuracy', n_jobs=-1,
            ).mean())
        except Exception:
            lda_acc = float('nan')

        # LinearSVC
        svc_acc = float(cross_val_score(
            make_pipeline(
                StandardScaler(),
                LinearSVC(C=CONFIG["svc_C"], max_iter=3000,
                          random_state=CONFIG["seed"]),
            ),
            Xs, y, cv=skf, scoring='accuracy', n_jobs=-1,
        ).mean())
...
# ELBOW DETECTION
# =====================================================================
def find_elbow(counts: list, accs: list) -> int:
    """
    Returns the smallest band count k that achieves at least
    CONFIG['elbow_pct'] × peak_accuracy.
    ...
    """
    peak      = max(accs)
    threshold = CONFIG["elbow_pct"] * peak
...
# =====================================================================
def save_outputs(final_bands: np.ndarray,
                 wl_df: pd.DataFrame,
                 tag: str):
    """
    ...
    for s in tqdm(range(0, N, CONFIG["chunk_size"]), desc="       writing"):
        e = min(s + CONFIG["chunk_size"], N)
...
def main():
    ...
    # ── Load ──────────────────────────────────────────────────────
    wl_df      = pd.read_csv(CONFIG["wavelength_path"])
    X, y       = extract_mean_spectra()

    # ── Pre-filter ────────────────────────────────────────────────
    candidates = decorrelation_prefilter(X)
    ...
    mrmr_order, rel_local = run_mrmr(X, y, candidates)

    # ── SPA — seeded from highest-MI band ────────────────────────
    best_mi_global = candidates[int(np.argmax(rel_local))]
    spa_order      = run_spa(X, candidates, init_global=best_mi_global)

    # ── Validation ────────────────────────────────────────────────
    print(f"\n  [5b/6]  Cross-validated accuracy curves...")
    mrmr_val = validate(X, y, mrmr_order, "mRMR")
    spa_val  = validate(X, y, spa_order,  "SPA")

    # ── Compare methods ───────────────────────────────────────────
    shared = [k for k in CONFIG["n_candidates"]
              if k in mrmr_val and k in spa_val]
    ...
    optimal_k   = find_elbow(shared, w_accs)
    ...
    rpath  = Path(CONFIG["output_dir"]) / "band_selection_report.csv"
    report.to_csv(rpath, index=False)
    print(f"\n  Accuracy table:\n{report.to_string(index=False)}")
    print(f"\n  Report → {rpath}")

    # ── Save reduced dataset ──────────────────────────────────────
    save_outputs(final_bands, wl_df, winner)

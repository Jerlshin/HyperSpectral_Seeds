...
def _load_data_mmap(patches_path: str, labels_path: str) -> None:
    ...
    # mmap_mode='r' keeps the data on disk, reading only on-demand
...
class DifferentiableSpectralRouter(nn.Module):
    """
    Advanced Differentiable Hard Band Selector.
    Uses Spectral Self-Attention to find correlated/redundant bands, and a 
    Straight-Through Gumbel-Sigmoid estimator to physically drop bands (multiply by exact 0.0) 
    while remaining completely end-to-end trainable.
    """
    def __init__(self, channels: int, d_model: int = 32, n_heads: int = 4) -> None:
        super().__init__()
        self.channels = channels
        
        # 1. Project global stats (mean, max, std) into an embedding space per band
        self.stat_proj = nn.Linear(3, d_model)
        
        # 2. Multi-Head Self-Attention allows bands to compare themselves to other bands
        # to identify highly redundant or highly informative regions.
        self.self_attn = nn.MultiheadAttention(embed_dim=d_model, num_heads=n_heads, batch_first=True)
        
        # 3. Predict logits for the keep/drop decision
        ...
        # Learnable temperature for the Gumbel distribution
        self.tau = nn.Parameter(torch.tensor(1.0))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, C, H, W = x.shape
        
        # --- A. Background-Aware Statistical Extraction ---
        ...
        # Shape: (Batch, Channels, 3)
        stats = torch.stack([x_mean, x_max, x_std], dim=-1)
        
        # --- B. Spectral Self-Attention ---
        # Shape: (Batch, Channels, d_model)
        ...
        # --- C. Straight-Through Gumbel-Sigmoid ---
        if self.training:
            ...
            # hard_gate is exactly 0.0 or 1.0. 
            # By subtracting and adding soft_gate.detach(), the forward pass uses the hard 0/1, 
            # but the backward pass routes gradients through the soft_gate.
            hard_gate = (soft_gate > 0.5).float()
            gate = hard_gate.detach() - soft_gate.detach() + soft_gate
        else:
            # During inference, it becomes a deterministic hard gate
            gate = (logits > 0.0).float()
            
        # --- D. Apply Mask ---
        # Useless bands are physically zeroed out across the entire batch
...
# ══════════════════════════════════════════════════════════════════════
#  BRANCH D — SPECMAMBA (Bidirectional State Space Model)
# ══════════════════════════════════════════════════════════════════════

class BidirectionalMambaBlock(nn.Module):
    """
    Dual-scan Mamba block. Sweeps the spectrum forward (VIS->NIR) 
    and backward (NIR->VIS) to capture non-causal chemical dependencies.
    """
    def __init__(self, d_model: int, d_state: int = 16, d_conv: int = 4, expand: int = 2):
        super().__init__()
        self.norm = nn.LayerNorm(d_model)
        
        # Independent selective state spaces for both directions
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Forward scan
        fwd = self.mamba_fwd(normed_x)
        
        # Backward scan (flip sequence dimension, process, flip back)
        bwd = self.mamba_bwd(normed_x.flip(dims=[1])).flip(dims=[1])
        
        # Residual fusion
        return x + fwd + bwd

class SpecMambaBranch(nn.Module):
    """
    Advanced Spatio-Spectral Mamba Branch.
    Treats the 16 spatial grid regions as joint features across the spectral sequence.
    """
    def __init__(
        self,
        physical_wl: torch.Tensor,
        num_bands: int = 256,
        num_regions: int = 16, # 4x4 spatial grid
        d_model: int = 128,
        n_layers: int = 4,
        out_dim: int = 256,
        dropout: float = 0.15,
        ...
        # 1. Joint Spatio-Spectral Embedding
        # Ingests all 16 regions simultaneously per wavelength step.
        self.embed = nn.Sequential(
            nn.Conv1d(num_regions, d_model, kernel_size=7, stride=2, padding=3, bias=False),
            nn.GroupNorm(1, d_model),
            nn.GELU()
        )
        self.seq_len = num_bands // 2 

        # 2. Physical Wavelength Positional Encoding
        dev = physical_wl.device
        pooled_wl = F.avg_pool1d(physical_wl.view(1, 1, -1), kernel_size=2, stride=2).squeeze()
        
        pe = torch.zeros(self.seq_len, d_model, device=dev)
        half = d_model // 2
        freq = torch.exp(torch.arange(half, device=dev).float() * -(math.log(1e4) / max(half - 1, 1)))
        pe[:, :half] = torch.sin(pooled_wl.unsqueeze(1) * freq.unsqueeze(0))
        pe[:, half:] = torch.cos(pooled_wl.unsqueeze(1) * freq.unsqueeze(0))
        self.register_buffer("wl_pe", pe)

        # 3. Bidirectional Mamba Sequence Processing
        self.layers = nn.ModuleList([
            BidirectionalMambaBlock(d_model=d_model, d_state=16, d_conv=4, expand=2)
    ...
    def forward(self, grid_ms: torch.Tensor) -> torch.Tensor:
        # grid_ms shape: [Batch, 16, 256]
        # Conv1d expects [Batch, Channels, Sequence_Length], which fits perfectly!
        x = self.embed(grid_ms).transpose(1, 2)  # -> [Batch, 128, d_model]
        ...
        attn_weights = torch.softmax(self.attn_pool(x), dim=1)  # [Batch, 128, 1]
        x_pooled = (x * attn_weights).sum(dim=1)                # [Batch, d_model]
        
        return self.proj(x_pooled)
...
class DynamicGradientOrthogonalization(torch.autograd.Function):
    ...
    def forward(ctx, b_a, b_b, b_c, b_d, log_a, log_b, log_c, log_d, targets):
        # 1. Evaluate confidence of each branch independently
        confidences = []
        with torch.no_grad():
            for logits in [log_a, log_b, log_c, log_d]:
                prob = torch.softmax(logits, dim=1)
                conf = prob[torch.arange(prob.size(0)), targets].mean().item()
                confidences.append(conf)
        
        # 2. Identify the index of the dominant branch
        ctx.dom_idx = int(np.argmax(confidences))
        
        # Return features untouched for the forward pass
    ...
    def backward(ctx, g_a, g_b, g_c, g_d):
        ...
        for i, g in enumerate(grads):
            if i == dom_idx:
                # The dominant branch keeps its full gradient
                out_grads.append(g)
            else:
                # 3. Project weaker gradients orthogonal to the dominant gradient
                dot_product = (g * g_dom).sum(dim=1, keepdim=True)
                norm_sq = (g_dom * g_dom).sum(dim=1, keepdim=True) + 1e-8
                
                projection = (dot_product / norm_sq) * g_dom
                g_orthogonal = g - projection
                
                out_grads.append(g_orthogonal)
                
        # MUST return 9 items to match the 9 inputs in forward()
        # The last 5 correspond to the 4 aux_logits and the 1 target tensor, which need no gradients here.
        return out_grads[0], out_grads[1], out_grads[2], out_grads[3], None, None, None, None, None

def apply_goal(ba, bb, bc, bd, aux_out_dict, targets):
    if not ba.requires_grad or targets is None:
        return ba, bb, bc, bd
    
    # Unpack the dictionary directly into the apply function
    return DynamicGradientOrthogonalization.apply(
        ba, bb, bc, bd, 
        aux_out_dict["aux_a"], aux_out_dict["aux_b"], 
        aux_out_dict["aux_c"], aux_out_dict["aux_d"], 
        targets
    )
...
class SpectralQuadNet(nn.Module):
    ...
    def __init__(
        ...
        self.branch_d = SpecMambaBranch(
            physical_wl=_PHYSICAL_WL,
            num_bands=num_bands,
            d_model=cfg.get("specf_dim", 128),
            n_layers=cfg.get("specf_layers", 4),
            out_dim=256,
            dropout=0.10,
        )
    ...
    def forward(
        ...
        if branch_mask is not None:
            ...
            drop_probs = torch.tensor([0.001, 0.001, 0.4, 0.01], device=ba_raw.device)
        ...
        # --- Gradient Orthogonalization (GOAL) ---
        aux_out = {}
        if self.training:
            # Get aux predictions early
            ...
            if labels is not None:
                # Pass the unpacked features and the dictionary directly
                ba, bb, bc, bd = apply_goal(ba_raw, bb_raw, bc_raw, bd_raw, aux_out, labels)
            else:
                ba, bb, bc, bd = ba_raw, bb_raw, bc_raw, bd_raw
        else:
            ba, bb, bc, bd = ba_raw, bb_raw, bc_raw, bd_raw

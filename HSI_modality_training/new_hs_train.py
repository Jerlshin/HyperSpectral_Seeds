...
CONFIG: dict = {
    ...
    "s1_epochs":            286,        # Tuned from 400
    "s1_phase1_frac":       0.2102,     # Increased for longer exploration
    "s1_phase2_frac":       0.4171,
                                 
    "s1_batch":             128,
    "s1_max_lr":            5.97e-4,
    "s1_min_lr":            3.09e-7,
    "s1_dropout":           0.2520,     # Increased for better regularization
    "s1_mixup":             0.3800,
    "s1_patience":          143,        # Scaled with epochs
    "s1_accum":             2,          # Effective batch size of 256
    "s1_focal_gamma":       2.3416,
    "s1_label_smooth_hi":   0.0637,
    "s1_label_smooth_lo":   0.0128,
    "s1_ema_reinit_phases": True,

    # ── Stage 1 · Phase 3 — Hard-Class Oversampling ───────────────────
    "s1_p3_oversample":         False,  # HPO found standard sampling better
    "s1_p3_oversample_power":   0.40,
    "s1_p3_oversample_max_w":   5.0,
    "s1_p3_hard_f1_thresh":     0.6149,
    "s1_p3_oversample_eps":     0.1282,   

    # ── Architecture ──────────────────────────────────────────────────
    "branch_drop_prob":    0.3001,      # Higher branch dropout for ensemble effect
    "subcenter_K":          4,          # Increased from 3
    "max_cutout_bands":     8,
    "noise_std":            0.0506,     # Slightly more noise for robustness

    # ── Auxiliary Classification Heads (per branch, Stage 1) ──────────
    "aux_head_hidden":       128,
    "aux_loss_weight_init":  0.7010,
    "aux_loss_weight_final": 0.1418,

    # ── Stage 2 (Using S1-Tuned ArcFace Params) ───────────────────────
    "s2_epochs":            120,
    "s2_batch":             128,
    "s2_head_lr":           2.5e-4,
    "s2_back_lr":           2.5e-5,
    ...
    "s2_dropout":            0.10,
    "s2_patience":           40,
    "s2_arcface_s":         31.4221,    # Tuned
    "s2_arcface_m":          0.3672,    # Tuned
    "s2_arcface_m0":         0.02,
    "s2_arcface_m_delta":    0.1178,    # Tuned
    ...
    # ── Shared ────────────────────────────────────────────────────────
    "weight_decay":          4.2298e-5, # Significantly lower than original 2e-4
    "grad_clip":              0.5088,   # Tighter clipping
    "ema_decay":             0.9992,

    # ── Transformer Branch (SpecFormer) ───────────────────────────────
    "wl_embed_dim":           8,        # Reduced from 16
    "specf_patch":            32,
    "specf_dim":             192,       # Leaner representation
    "specf_heads":             8,
    "specf_layers":            4,
    "specf_drop":             0.0523,
    "fusion_heads":            4,
...
class SpectralQuadNet(nn.Module):
    ...
    def forward(
        ...
        if branch_mask is not None:
            ...
            drop_probs = torch.tensor([0.05, 0.05, 0.25, 0.10], device=ba_raw.device)

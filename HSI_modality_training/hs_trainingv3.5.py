...
CONFIG: dict = {
    ...
    "s1_epochs":            300,
    "s1_phase1_frac":       0.25,   # Phase 1: 0 → 25%  (heavy aug + mixup)
    "s1_phase2_frac":       0.35,   # Phase 2: 25 → 60% (medium aug + mixup)
                                    # Phase 3: 60 → 100% (light aug, focal, oversample)
    "s1_batch":             128,
    "s1_max_lr":            2e-3,
    "s1_dropout":           0.10,
    "s1_mixup":             0.10,
    ...
    # Each of the four spectral branches gets its own lightweight
    # classification head for deep supervision during Stage 1 only.
    "aux_head_hidden":       128,   # hidden dimension inside each aux head
    "aux_loss_weight_init":  0.50,  # weight at epoch 0  (decays linearly)
    "aux_loss_weight_final": 0.15,  # weight at final epoch
    ...
    "s3_sam_rho":             0.05,
    "s3_greedy":            True,
    "s3_aux_loss_weight":    0.10,   # fixed aux weight during SAM training

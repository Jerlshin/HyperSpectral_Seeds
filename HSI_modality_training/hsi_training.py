...
CONFIG: dict = {
    ...
    "output_dir":      "/Users/jerlshin/FieldOfInterest/ResearchWork/HSI_RGB_seeds/Code/HSI_modality_training/output_v12_SPA40",
    ...
    "s1_accum":             1,
    "s1_focal_gamma":       1.5,
    "s1_label_smooth_hi":   0.10,
    "s1_label_smooth_lo":   0.04,
    "s1_ema_reinit_phases": True,

    # ── Stage 1 · Phase 3 contrastive losses ─────────────────────────
    "s1_p3_supcon_weight":  0.35,
    "s1_p3_proto_weight":   0.15,

    # ── Stage 1 · Phase 3 — Hard-Class Oversampling ───────────────────
    "s1_p3_oversample":         True,
    "s1_p3_oversample_power":   0.65,
    "s1_p3_oversample_max_w":   7.0,
    "s1_p3_hard_f1_thresh":     0.80,
...
class SpectralProfileBranch(nn.Module):
    def __init__(
        ...
        self.tower_s = nn.Sequential(LargeKernelBlock1D(tower_ch, 3), LargeKernelBlock1D(tower_ch, 3))
        self.tower_m = nn.Sequential(LargeKernelBlock1D(tower_ch, 5), LargeKernelBlock1D(tower_ch, 5))
        self.tower_l = nn.Sequential(LargeKernelBlock1D(tower_ch, 7), LargeKernelBlock1D(tower_ch, 7))
...
def _aux_loss_weight(current_ep: int, total_ep: int) -> float:
    ...
    return max(
        ...
        CONFIG["aux_loss_weight_init"] * (1.0 - progress * 0.7),  # slower decay
    )
...
def run_stage1(
    ...
    # Custom Phase-Aware Scheduler
    def phase_aware_lr(ep_idx):
        ep = ep_idx + 1
        ...
        # Shorter cycles (30 eps) give more restarts in the same 193-ep window.
            ...
            min_ratio = CONFIG["s1_min_lr"] / CONFIG["s1_max_lr"]
            mid_ratio = CONFIG.get("s1_mid_lr", 2.5e-4) / CONFIG["s1_max_lr"]
            cosine_val = 0.5 * (1.0 + math.cos(math.pi * cycle_ep / CYCLE))
            return min_ratio + (mid_ratio - min_ratio) * cosine_val

    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        scheduler = optim.lr_scheduler.LambdaLR(optimizer, phase_aware_lr)
    ...
    print(f"{'═'*w}")
    print(f"  Phase 1: ep 1–{p1_end}         heavy aug + mixup (α={CONFIG['s1_mixup']})")
    ...
    print(f"  P3 contrastive: SupCon(w={CONFIG['s1_p3_supcon_weight']}) "
          f"ProtoNCE(w={CONFIG['s1_p3_proto_weight']})")
    print(f"  Branch aux weights: A/B×2.0  C/D×1.0  |  Drop probs: A=0 B=0 C=0.30 D=0.20")
    ...
    for ep in range(1, ep_total + 1):
        ...
        if phase == 3 and not ema_reinited[1] and CONFIG["s1_ema_reinit_phases"]:
            ...
            p3_drop = CONFIG.get("s1_p3_dropout", 0.25)
        ...
        sc_w_now = CONFIG["s1_p3_supcon_weight"] if phase == 3 else 0.0
        pt_w_now = CONFIG["s1_p3_proto_weight"]  if phase == 3 else 0.0
...
def main() -> None:
    ...
    if done_stage < 2:
        ...
        tr2, va2, _ = build_loaders(
            ...
            train_aug="very_light", class_weights=cdws_wts_s1,
        )

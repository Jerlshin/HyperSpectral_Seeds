"""
HSI Rice Seed Classification ─ Refined Pipeline v2
====================================================
Dataset : 90 rice varieties × ~96 kernels each (~8640 total)
Input   : patches.npy  →  (N, 256, 64, 64)  float16  channel-first
          labels.npy   →  (N,)               int64

═══════════════════════════════════════════════════════════════════
ROOT CAUSE OF PREVIOUS FAILURE (0% train accuracy)
═══════════════════════════════════════════════════════════════════
ArcFace(s=30, m=0.50) with random init creates an infeasible loss
landscape from the very first step:

  At init: θ ≈ π/2  (random unit vectors are near-orthogonal)
  Target logit  = 30·cos(π/2 + 0.5) = 30·(−sin 0.5) ≈ −14.4
  Other logits  = 30·cos(π/2)        ≈  0

  CE loss = 14.4 + log(89) ≈ 18.9  →  exactly the ~19.5 you see at Ep 1.

  The gradient from the margin penalty always points AWAY from the correct
  class, so the optimiser immediately lands in a saddle point at loss ≈ 10.9
  (log-uniform over 90 classes × s=30/3) that it can never escape.
  Result: 0% training accuracy for 110 epochs.

ArcFace needs millions of samples per class (face recognition) and a
careful pre-training + fine-tune warm-up.  For ≈67 samples/class it is
the wrong loss function.

═══════════════════════════════════════════════════════════════════
FIXES IN THIS VERSION
═══════════════════════════════════════════════════════════════════
1.  ArcFace removed → CrossEntropyLoss + label_smoothing=0.1
2.  Mixup augmentation (α=0.4) for fine-grained 90-class problem
3.  Architecture rebalanced: spectral branch is PRIMARY discriminant
    for seed varieties (each variety has a unique spectral fingerprint)
4.  Masked-mean spectrum (ignores zeroed background pixels)
5.  Multi-scale 1D spectral CNN + lightweight 2D spatial CNN
6.  Cosine LR with linear warm-up + gradient clipping
7.  Fixed syntax error in Dataset (extra closing parenthesis)
8.  Auto-create output directory
9.  Full test-set evaluation with classification report

Expected performance after ~100 epochs: Val Acc 60–85%
"""
